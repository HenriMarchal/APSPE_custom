...
class main_controller
{
	...
	public function submit($user_id, $admin, $auto_login, $viewonline, $class)
	{
		...
		{
			error_log('[TFA] tfa_random is empty');
			throw new http_exception(400, 'TFA_SOMETHING_WENT_WRONG4');
		}
		if ($user_id != $this->user->data['tfa_uid'])
		{
			error_log('[TFA] tfa_uid does not match: '.$this->user->data['tfa_uid'].' and user_id is: '.$user_id);
			throw new http_exception(400, 'TFA_SOMETHING_WENT_WRONG5');
		}
		$random = $this->request->variable('random', '');

		if ($this->user->data['tfa_random'] !== $random)
		{
			error_log('[TFA] tfa_random does match: '.$this->user->data['tfa_random'].' and $random is: '.$random);
			throw new http_exception(400, 'TFA_SOMETHING_WENT_WRONG6');
		}
		if (strlen($random) !== 40)
		{
			error_log('[TFA] random lenght is not 40: '.strlen($random));
		...
		if (empty($class))
		{
			error_log('[TFA] the class is empty');
			throw new http_exception(400, 'TFA_SOMETHING_WENT_WRONG8');
		}
		...
		if ($module == null)
		{
			error_log('[TFA] the module is null');
			throw new http_exception(400, 'TFA_SOMETHING_WENT_WRONG9');
		}
