...
class article
{
	...
	public function handle($alias = 'index', $param = 0)
	{
			...
			else
			{
				...
				foreach ($children as $child)
				{
					...
					$this->template->assign_block_vars('category_children', array(
						...
						'TITLE'			 => $child['title'],
						'FEATURED_IMG'	 => empty($child['featured_img']) ? $this->cmbb->phpbb_user_avatar($child['user_id']) : $this->cmbb->get_featured_img($child['featured_img'], $child['title']),
			...
			if (isset($trail))
			{
				...
				foreach ($trail as $crumb)
				{
					$this->template->assign_block_vars('cmbb_crumbs', array(
						'U_CRUMB_LINK'	 => append_sid($crumb['alias']),
		...
		$this->template->assign_vars(array(
			...
			'CMBB_ARTICLE_TOPIC_URL' => append_sid('viewtopic.php?t='.$article['topic_id']),
