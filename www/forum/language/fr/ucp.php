...
$lang = array_merge($lang, [
	...
	<br><br>
	Vous acceptez de ne publier aucun contenu à caractère abusif, obscène, vulgaire, diffamatoire, choquant, menaçant, pornographique, etc. qui pourrait transgresser la législation française (où notre serveur est hébergé). Si vous ne respectez pas ces dispositions, vous vous exposez à un bannissement immédiat et définitif et nous nous réservons le droit d’avertir votre fournisseur d’accès à internet et les autorités officielles. L’adresse IP de tous les messages est enregistrée afin d’aider au renforcement de ces conditions. Vous acceptez le fait que « %1$s » ait le droit de supprimer, de modifier, de déplacer ou de verrouiller n’importe quel sujet et message à n’importe quel moment si nous estimons cela nécessaire. En tant qu’utilisateur, vous acceptez que toutes les informations que vous avez renseignées soient enregistrées dans notre base de données. Bien que ces informations ne seront pas diffusées à une tierce partie sans votre consentement, ni « %1$s », ni phpBB, ne pourront être tenus comme responsables en cas de tentative de piratage informatique visant à compromettre vos données.<br>
...
$lang = array_merge($lang, [
	...
	'ACCOUNT_INACTIVE_ADMIN'         => 'Votre compte a été créé. Cependant, un administrateur doit activer votre compte. Vous serez informé lors de l’activation de votre compte. En attendant, vous accès en lecture seule au forum mais ne pouvez pas vous connecter au tchat',
